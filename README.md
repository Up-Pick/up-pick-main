...
- DAU (일일 활성 사용자): 15,000명 (MAU의 15%)
- 최대 동시 접속자: 750 ~ 1,500명 (DAU의 5% ~ 10%)
- Think Time: 5초 ~ 15초
- 평균 TPS: 150
- 목표 TPS: 300
...
### find-max-tps-test.js

서버의 최대 TPS 한계를 찾는 스트레스 테스트입니다.
...
- 단계별 TPS 증가: 50 → 100 → 150 → 200 → 250 → 300
- 각 단계 지속 시간: 1분
- 총 테스트 시간: 6분
- 최대 VU: 1,500명
...
|-------|------|--------------------|
| p(50) | < 1초 | 평균적인 응답 시간         |
| p(95) | < 2초 | 대부분의 요청이 만족해야 할 시간 |
| p(99) | < 5초 | 극단적인 경우를 제외한 최대 시간 |
...
- `http_req_duration`: HTTP 요청의 전체 소요 시간
- `http_req_failed`: 실패한 요청의 비율
- `http_reqs`: 총 요청 수
- `vus`: 가상 사용자 수 (Virtual Users)
- `vus_max`: 최대 가상 사용자 수

### 커스텀 메트릭

- `errors`: 커스텀 에러율 (체크 실패 포함)
- `total_requests`: 전체 요청 카운터
- `product_detail_duration`: 상품 상세 조회 응답 시간
- `product_simple_duration`: 상품 간단 조회 응답 시간
- `search_product_duration`: 상품 검색 응답 시간
