...
public class DefaultMemberInnerService implements MemberInnerService {
	...
	public boolean existsByEmail(String email) {
		...
		log.debug("DefaultMemberInnerService - 이메일 존재 확인 시도");

		boolean exists = memberRepository.existsByEmail(email);

		log.debug("DefaultMemberInnerService - 이메일 존재 확인 성공 ✅");
	...
	public boolean existsByNickname(String nickname) {
		...
		log.debug("DefaultMemberInnerService - 닉네임 존재 확인 시도 ⏳");

		boolean exists = memberRepository.existsByNickname(nickname);

		log.debug("DefaultMemberInnerService - 닉네임 존재 확인 성공 ✅");
	...
	public void createUser(SignupRequest request, String encodedPassword) {
		...
		log.debug("DefaultMemberInnerService - 멤버 생성 시도 ⏳");

		Member member = memberMapper.toEntity(request, encodedPassword);
		memberRepository.save(member);

		log.debug("DefaultMemberInnerService - 멤버 생성 성공 ✅");
	}

	@Override
	public Member findByEmail(String email) {

		log.debug("DefaultMemberInnerService - 이메일로 멤버 조회 시도 ⏳");

		Member member = memberRepository.findByEmail(email)
			.orElseThrow(() -> new BusinessException(AuthErrorCode.USER_NOT_FOUND));

		log.debug("DefaultMemberInnerService - 이메일로 멤버 조회 성공 ✅");
