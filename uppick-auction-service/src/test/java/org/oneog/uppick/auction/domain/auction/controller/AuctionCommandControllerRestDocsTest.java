...
import org.oneog.uppick.auction.domain.auction.command.controller.AuctionCommandController;
...
@WebMvcTest(AuctionCommandController.class)
@WithMockAuthMember(memberId = 1L)
class AuctionCommandControllerRestDocsTest extends RestDocsBase {
	...
	void auctionBid_정상적인입력_입찰성공() throws Exception {
		...
		doNothing().when(auctionCommandService).bid(any(AuctionBidRequest.class), eq(auctionId), eq(memberId));
